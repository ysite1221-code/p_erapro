...
// verification_status ラベル
function kyc_label($status) {
    return match((int)$status) {
        0 => ['未提出',   '#aaa',    '#f5f5f5'],
        1 => ['審査待ち', '#d39e00', '#fff8e1'],
        2 => ['承認済み', '#2e7d32', '#f0faf2'],
        9 => ['否認',     '#c62828', '#fff0f0'],
        default => ['不明', '#999', '#f5f5f5'],
    };
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Agent一覧 - ERAPRO Admin</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .btn-kyc:hover { background: #007bff; color: #fff; border-color: #007bff; }
...
<div class="sidebar">
    ...
    <div class="menu">
        ...
        <a href="admin_agent_list.php" class="active"><span class="material-icons-outlined icon">people</span>Agent一覧</a>
...
<div class="content">
    ...
    <form method="get" class="search-bar">
        ...
        <?php if ($search !== ''): ?>
        <a href="admin_agent_list.php"><button type="button" class="btn-clear">クリア</button></a>
        <?php endif; ?>
    </form>
    ...
    <div class="table-card">
        ...
        <table>
            ...
            <tbody>
                <?php if (empty($agents)): ?>
                <tr class="empty-row"><td colspan="7">該当するAgentが見つかりませんでした。</td></tr>
                <?php else: ?>
                <?php foreach ($agents as $a):
                    [$kyc_text, $kyc_color, $kyc_bg] = kyc_label($a['verification_status']);
                    $is_active = ((int)$a['life_flg'] === 0);
                ?>
                <tr>
                    <td style="color:#aaa; font-size:0.78rem;">#<?= h($a['id']) ?></td>
                    <td><strong><?= h($a['name']) ?></strong></td>
                    <td style="color:#666;"><?= h($a['lid']) ?></td>
                    <td><?= h($a['area'] ?: '—') ?></td>
                    <td>
                        <span class="badge" style="background:<?= $kyc_bg ?>; color:<?= $kyc_color ?>;">
                            <?= $kyc_text ?>
                        </span>
                        <?php if ((int)$a['verification_status'] === 1): ?>
                        <a href="admin_kyc_check.php?id=<?= (int)$a['id'] ?>" class="btn-kyc">審査する</a>
                        <?php endif; ?>
                    </td>
                    <td>
                        <?php if ($is_active): ?>
                    ...
                    <td>
                        <form method="post" action="admin_status_act.php" style="display:inline;"
                              onsubmit="return confirm('<?= $is_active ? 'このAgentを停止しますか？' : 'このAgentを再開しますか？' ?>');">
